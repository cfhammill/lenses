...
If we want to apply a function to change the data, we
can apply a function `over` the lens:
...
1. `set`: set the subpart to a particular value, then return the
  whole object with the subpart updated.
1. `over`: apply a function to the subpart, then return the 
  whole object with the subpart updated.
1. `%.%`: compose two lenses to focus on a subpart of a subpart.
...
For example, `lenses` provides lens-ified version of `dplyr::select`.
Since lenses are "bidirectional", this means you can `set` the results
of your selection.
...
universe. But our lens-ified `filter` does it with ease.
...
workflows, as well as your base R workflows.
...
rules (the "Lenz laws", here paraphrased from [a Haskell lens tutorial](
